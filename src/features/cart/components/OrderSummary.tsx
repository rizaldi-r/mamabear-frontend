...
export const OrderSummary = ({
  ...
  handleApplyPromo,
  selectedCount,
  handleCheckout // <-- Receive the new prop
}: {
  subtotal: number;
  ...
  handleCheckout: () => void; // <-- Type definition
}) => {
  return (
    <div className="border border-gray-200 rounded-2xl p-6 bg-white sticky top-24 shadow-sm">
      ...
      <button 
        ...
        className="w-full bg-[var(--mama-hot-pink)] text-white py-4 rounded-full font-bold text-font-4 hover:opacity-90 transition-opacity disabled:opacity-50 shadow-md"
      >
        CHECK OUT
      </button>
    </div>
