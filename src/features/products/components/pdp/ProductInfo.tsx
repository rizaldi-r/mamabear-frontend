...
function ProductInfo({ product, avgReview, totalReview }: ProductProps) {
  ...
  function handleAddStock() {
    ...
    if (qty >= stock) {
      ...
      setAlert(
        "Dapatkan harga khusus untuk pembelian >10. Hubungi Admin MamaBear.",
      );
      setQty(10);
    } else {
      setQty(qty + 1);
  ...
  function submitProduct() {
    try {
      ...
      const stored = localStorage.getItem("cart-item");

      let prevCart = [];

      try {
        const parsed = stored ? JSON.parse(stored) : [];
        prevCart = Array.isArray(parsed) ? parsed : [];
      } catch {
        prevCart = [];
      }
      ...
      const newItem = {
        ...
        qty: qty,
        priceIdr: selectedVariant.priceIdr,
  ...
  return (
    ...
    {/* REVIEW STARS */}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2 w-[70%]">
            {variants.map((v) => (
              <p
                key={v.id}
                className={`py-1 px-5 rounded-2xl font-bold cursor-pointer
                          ${
                            v.stock === 0
      ...
      {stock == 0 && (
        <p>
          Maaf, stok habis 😔 Subscribe atau follow Instagram MamaBear untuk
          mendapatkan info saat stok kembali tersedia.
        </p>
      )}
      ...
      <div className="flex gap-2">
        <button
          ...
          className="w-[90%] bg-[var(--mama-hot-pink)] text-font-4 rounded-4xl font-bold text-white flex items-center justify-center"
        >
          <svg
